...
char prompt[] = "CSC256Shell$ ";    /* command line prompt (DO NOT CHANGE) */
...
void eval(char *cmdline)
{
    ...
    if(!builtin_cmd(argv)) {
        ...
        //store the index of "|" operators
        int index[argc];
        memset(index,'\0',argc*sizeof(index[0]));
        int pipe_no = 0;
        ...
        if(pipe_no){
            ...
            for(int p=0; p<=pipe_no-1;p++){
                ...
                int k=0;
                /* copy arguments of the command before pipe to the new temp variable */
//                printf("debug: %d\n", i);
                for(;i < index[p];i++){
                    argv1[k] = malloc(sizeof(argv[i]));
                    if(i < index[p]){
            ...
            char ** argv1 = malloc(sizeof(char *)*pipe_no);
            int i = index[pipe_no-1]+1;
            int k=0;
            /* copy arguments of last command to argv1*/
            for(;i < argc;i++){
                argv1[k] = malloc(sizeof(argv[i]));
                if(i < argc){
                    strcpy(argv1[k],argv[i]);
...
int builtin_cmd(char **argv)
{
    //if the argument is exit, then use exit(0) to terminate the shell normally.
    if(!strcmp("exit",argv[0])){
        exit(0);
    }

    //if the argument is fg or bg, then call the do_bgfg function to deal with it.
    if(!strcmp("fg",argv[0]) || !strcmp("bg",argv[0])){
        do_bgfg(argv);
        return 1;
    }

    //if the argument is jobs, then call the listjobs function to print all the jobs we stored.
...
void listjobs(struct job_t *jobs)
{
    ...
    if (jobs[i].pid != 0) {
        printf("[%d]: ", jobs[i].jid);
        // the MP1 program did not require to print out the state of the job
//        switch (jobs[i].state) {
//        case BG:
//            printf("Running ");
//            break;
...
/*
* cd - the biult in function to deal with cd commands.
*/
void cd(char** argv){
    char * temp = argv[1];
    if(temp==NULL){ /* if the input arguments only has one argumnent, then it will automatically cd into home directory */
        temp = getenv("HOME");
        if(temp == NULL){ //for cases that cannot get home dir, then print the following
            printf("%s: get home directory failed: %s\n", argv[0], strerror(errno));
            return;
        }
    }

    //execute chdir function call and print out error message if failed
    if(chdir(temp) == -1){
           printf("%s %s: cd failed: %s\n", argv[0], argv[1], strerror(errno));
//        getwd(temp);
//        printf("\n\n\n%s", temp);
        return;
       }
        char curr_dir[1001];
        memset(curr_dir, 0, 1001);
            getwd(curr_dir);
            printf("%s ", curr_dir);
    return;
}
